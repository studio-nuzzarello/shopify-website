  ...
  _toggleExpanded(target, items) {
    ...
    if (target.getAttribute("data-type") === "reset") {
      ...
      if (item.getAttribute(attributes.expandedTarget) === "false") {
        setTimeout(function () {
          const element = document.getElementById("AddAddress");
          if (element) {
            const topPosition = element.offsetTop - 86;
            window.scrollTo({
              top: topPosition,
              behavior: "smooth",
            });
...
(() => {
  window.addEventListener("load", function () {
    ...
    if (selects.length > 0) {
      selects.forEach((selectContiner) => {
        const select = selectContiner.querySelector("select");
        if (!select) return;
